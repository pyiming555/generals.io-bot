...
def run(n_a, n_b, ng, label):
    ...
    total = ng * 2
    print(f"\n  {label} ({total}局)")
    print(f"  n={n_a}: {wa}胜 ({wa/total*100:.0f}%)  {ta/ca*1000:.2f}ms/步")
    print(f"  n={n_b}: {wb}胜 ({wb/total*100:.0f}%)  {tb/cb*1000:.2f}ms/步")

run(300, 200, 25, "NN+MCTS(n=300) vs NN+MCTS(n=200)")
lib.nn_destroy(nn)
print("\n✅ 完成!")
